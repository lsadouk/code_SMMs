...
%% ----------------------------------------------
%% code added for feature representation to display confusion matrix -> 1062=batch size, 2=nb labels
try
nb_labels = 2;
batch_size_test = opts.size_testLabel;
best_p = reshape(predictions,nb_labels,batch_size_test);
Y = bsxfun(@eq, (1:nb_labels)' * ones(1, batch_size_test), best_p(1,:)); %added
Ylabel = bsxfun(@eq, (1:nb_labels)' * ones(1, batch_size_test), reshape(labels,1,batch_size_test)); %added
...
accuracy = (tn+tp)/batch_size_test;
fprintf('F1 score %f\n',F1_score)
fprintf('Accuracy %f\n',accuracy)
catch
end
...
err(2,1) = sum(sum(sum(mass .* min(error(:,:,1:2,:),[],3)))) ;
